...
-- Enable Row Level Security (RLS) for Brands
...
-- Email Subscription Table for Insight Alerts
...
-- Create Policies for public alerts subscription
CREATE POLICY "Allow public insert to insight_alerts" ON public.insight_alerts FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow authorized reads on insight_alerts" ON public.insight_alerts FOR SELECT USING (true);
`;
...
/**
 * Register an email subscriber for the Insight Alerts system.
 * Saves to Supabase 'insight_alerts' table if configured, otherwise saves to localStorage.
 */
export async function subscribeToInsightAlerts(email: string): Promise<{ success: boolean; mode: 'supabase' | 'local' }> {
  const cleanEmail = email.trim().toLowerCase();
  if (!cleanEmail) return { success: false, mode: 'local' };
  ...
  if (supabase) {
    try {
      ...
      if (error) {
        console.warn('Supabase subscription failed/duplicate, indexing locally.', error);
      } else {
        return { success: true, mode: 'supabase' };
      }
    } catch (err) {
      console.error('Supabase subscription exception:', err);
    }
  }

  // Local storage fallback
  try {
    const saved = localStorage.getItem('abi_insight_alerts');
    const list = saved ? JSON.parse(saved) : [];
    if (!list.includes(cleanEmail)) {
      list.push(cleanEmail);
      localStorage.setItem('abi_insight_alerts', JSON.stringify(list));
    }
    return { success: true, mode: 'local' };
  } catch (e) {
    console.error('Local subscription save failed:', e);
